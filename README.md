...
Build-order step 1 (repo structure, env config, AWS provisioning) is in
place: Python app skeleton with a working health check + test, and
Terraform for all MVP AWS resources except the ECS service itself (added
once there's a container image worth deploying). See
`docs/decisions/0001-initial-architecture-decisions.md` for the reasoning
behind the choices made in this pass, including a couple flagged as worth
revisiting before go-live (RDS deletion protection, NAT-less networking).

Nothing here talks to SP-API, Keepa, or a real database yet — that's
steps 2 and onward.
...
cd backend
python -m venv .venv
./.venv/Scripts/pip install fastapi "uvicorn[standard]" pydantic-settings boto3 pytest httpx
...
available to verify this increment.)
